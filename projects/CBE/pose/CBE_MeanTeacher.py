...
def main(mark, params=None):
    ...
    unlabeled_dataset = datasets.__dict__['PoseDataset'](targets, unlabeled_idxs, datasource.mean, datasource.std, args, True)
    ...
    for epo in range(args.start_epoch, args.epochs):
        ...
        total_loss, labeled_losses, consistency_losses, fd_loss, mc_losses, pl_acc, sim_mse = train(labeled_trainloader, unlabeled_trainloader, model, model_ema, optimizer, args)
        ...
        fmtc = '[{}/{} | lr: {} | pl_acc: {}, pl_mse: {}] total_loss: {}, labeled_loss: {}, consistency_loss: {}, fd_loss: {}, mc_loss: {}, test_loss: {}'
        logc = fmtc.format(format(epo + 1, '3d'),
                           ...
                           format(consistency_losses[-1], '.5f'),
                           format(fd_loss, '.8f'),
...
def train(labeled_loader, unlabeled_loader, model, model_ema, optimizer, args):
    ...
    consistency_loss_counters = [AvgCounter() for _ in range(args.stream_num + 1)]
    ...
    # region 2. Training
    model.train()
    if args.use_ema: model_ema.ema.train()
    for batch_idx in range(args.eval_step):
        ...
        try:
            ...
            inputs_u, targets_u, meta_u = next(unlabeled_iter)
        except:
            unlabeled_iter = iter(unlabeled_loader)
            # targets_u: Use only when verifying the quality of pseudo-labels
            inputs_u, targets_u, meta_u = next(unlabeled_iter)
        ...
        inputs = proj.data_interleave(torch.cat((inputs_x, inputs_u)), args.mu+1).to(args.device)
        targets_x = targets_x.to(args.device)
        weights_x = meta_x['kps_weight'].to(args.device)
        kps_warpmat_u = meta_u['kps_warpmat'].to(args.device)
        is_flip_u = meta_u['is_flip'].to(args.device)
        center_u = meta_u['center'].to(args.device)
        scale_u = meta_u['scale'].to(args.device)
        ...
        ms_logits_x, ms_logits_u, ms_features = [], [], []
        for stIdx in range(args.stream_num):
            logits_x, logits_u = proj.data_de_interleave_group2(ms_preds[:, :, stIdx], batch_size, args.mu+1)
            ms_logits_x.append(logits_x)
            ms_logits_u.append(logits_u)
            _, fs_p = proj.data_de_interleave_group2(ms_fs_p[:, :, stIdx], batch_size, args.mu+1)
            ms_features.append(fs_p)
        del ms_preds, ms_fs_p, _
        ms_logits_x = torch.stack(ms_logits_x, dim=0)  # torch.Size([5, 4, 3, 9, 64, 64])
        ms_logits_u = torch.stack(ms_logits_u, dim=0)  # torch.Size([5, 4, 3, 9, 64, 64])
        ms_features = torch.stack(ms_features, dim=0)  # torch.Size([5, 4, 3, 25, 64, 64])

        ms_preds_ema, _ = model_ema.ema(inputs)
        ms_logits_u_ema = []
        for stIdx in range(args.stream_num):
            _, logits_u_ema = proj.data_de_interleave_group2(ms_preds_ema[:, :, stIdx], batch_size, args.mu+1)
            ms_logits_u_ema.append(logits_u_ema)
        del ms_preds_ema, _
        ms_logits_u_ema = torch.stack(ms_logits_u_ema, dim=0)  # torch.Size([5, 4, 3, 9, 64, 64])
        # endregion
        ...
        targets_logits_u_ema = torch.mean(ms_logits_u_ema[:, :, -1], 0).clone()  # torch.Size([4, 9, 64, 64])
        targets_hm_ens = aug.affine_back(targets_logits_u_ema, kps_warpmat_u, is_flip_u)
        targets_pred_ens, _ = proc.kps_from_heatmap(targets_hm_ens.cpu(), center_u, scale_u, [args.out_res, args.out_res])
        ...
        sim_mse, sim_count = bus.prediction_similarity(ms_logits_u_ema[:, :, -1])
        ...
        # region 2.5 EMA consistency constraint
        if args.lambda_ens > 0:
            blank_stIdx, unblank_sum = batch_idx % args.stream_num, 0
            blank_idxs = [blank_stIdx, blank_stIdx+1][0:args.blank_num]
            consistency_loss_sum = torch.tensor(0.).to(args.device)
            for stIdx in range(args.stream_num):
                for nIsx in range(args.stack_num):
                    logits_u_s_hm_back = aug.affine_back(ms_logits_u[stIdx, :, nIsx].clone(), kps_warpmat_u, is_flip_u)
                    consistency_loss_val, kps_count = pose_criterion(logits_u_s_hm_back, targets_hm_ens.detach())
                    consistency_loss_counters[stIdx].update(consistency_loss_val.item(), kps_count)
                    if stIdx not in blank_idxs:
                        consistency_loss_sum += consistency_loss_val*kps_count
                        unblank_sum += kps_count
            consistency_loss = consistency_loss_sum / unblank_sum
            consistency_loss_counters[-1].update(consistency_loss.item(), unblank_sum)
            log_content += ' | loss_consistency: {}'.format(format(consistency_loss.item(), '.3f'))
        else:
            consistency_loss = torch.tensor(0.).to(args.device)
            for counter in consistency_loss_counters: counter.update(0., 1)
        # endregion
        ...
        if args.epo < args.ensemble_warmup:
            ...
            loss = args.lambda_la * labeled_loss + args.lambda_ens * consistency_loss + args.lambda_fd * fd_loss + args.lambda_mc * mc_loss
    ...
    consistency_losses = [counter.avg for counter in consistency_loss_counters]
    ...
    # endregion
    return total_loss_val, labeled_losses, consistency_losses, fd_loss_val, mc_losses, pl_acc_val, sim_mse_val
...
def init_args(params=None):
    ...
    # Model Setting
    parser.add_argument('--arch', default='Hourglass_MS', type=str, choices=['Hourglass', 'Hourglass_MS'], help='model name')
    ...
    parser.add_argument("--expend", default="False")
...
if __name__ == '__main__':
    main('CBE_MeanTeacher_20240610.1', {})
    pass