...
def main(mark, params=None):
    ...
    for epo in range(args.start_epoch, args.epochs):
        ...
        total_loss, labeled_loss, consistency_loss, pl_acc = train(labeled_trainloader, unlabeled_trainloader, model, model_ema, optimizer, args)
        ...
        fmtc = '[{}/{} | lr: {} | pl_acc: {}] total_loss: {}, labeled_loss: {}, consistency_loss: {}, test_loss: {}'
...
def train(labeled_loader, unlabeled_loader, model, model_ema, optimizer, args):
    ...
    consistency_loss_counter = AvgCounter()
    ...
    for batch_idx in range(args.eval_step):
        ...
        # region 2.2 forward
        ms_preds = model(inputs)  # [3*bs, nstack, stream, k, 64, 64], [3*bs, nstack, stream, 25, 64, 64]
        logits_x, logits_u = proj.data_de_interleave_group2(ms_preds, batch_size, args.mu+1)
        del ms_preds

        ms_preds_ema = model_ema.ema(inputs)
        _, logits_u_ema = proj.data_de_interleave_group2(ms_preds_ema, batch_size, args.mu+1)
        del ms_preds_ema, _
        # endregion

        # region 2.3 supervised learning loss
        labeled_loss, kps_count = pose_criterion(logits_x[:, -1], targets_x, weights_x)
        ...
        targets_hm_ens = aug.affine_back(logits_u_ema[:, -1].clone(), kps_warpmat_u, is_flip_u)
        ...
        if args.lambda_ens > 0:
            ...
            for nIsx in range(args.stack_num):
                logits_u_hm_back = aug.affine_back(logits_u[:, nIsx].clone(), kps_warpmat_u, is_flip_u)
                consistency_loss_val, kps_count = pose_criterion(logits_u_hm_back, targets_hm_ens.detach())
                consistency_loss_sum += consistency_loss_val
            consistency_loss = consistency_loss_sum / args.stack_num
            consistency_loss_counter.update(consistency_loss.item(), args.stack_num*kps_count)
            log_content += ' | loss_consistency: {}'.format(format(consistency_loss.item(), '.3f'))
        else:
            consistency_loss = torch.tensor(0.).to(args.device)
            consistency_loss_counter.update(0., 1)
        # endregion
        ...
        if args.epo < args.ensemble_warmup:
            ...
            loss = args.lambda_la * labeled_loss + args.lambda_ens * consistency_loss
    ...
    labeled_loss = labeled_loss_counter.avg
    consistency_loss = consistency_loss_counter.avg
    pl_acc_val = pl_acc_counter.avg.item()
    # endregion
    return total_loss_val, labeled_loss, consistency_loss, pl_acc_val
