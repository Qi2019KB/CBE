...
def train(labeled_loader, unlabeled_loader, model, model_ema, optimizer, scheduler, args):
    ...
    for batch_idx in range(args.eval_step):
        ...
        if args.debug and (args.epo+1) % 5 == 0:
            ...
            # region 2.10.2 pseudo-labels distribution visualization
            ...
            if (batch_idx + 1) in stat_bat_idxs:
                for stIdx in range(args.stream_num + 1):
                    if stIdx == args.stream_num:
                        ...
                        stat_pl_preds[stIdx] = preds_sat[stIdx] if stat_pl_preds[stIdx] is None else torch.cat([stat_pl_preds[stIdx], preds_sat[stIdx]])
                        stat_pl_masks[stIdx] = masks_sat[stIdx] if stat_pl_masks[stIdx] is None else torch.cat([stat_pl_masks[stIdx], masks_sat[stIdx]])
...
def init_args(params=None):
    ...
    parser.add_argument('--count-thr', default=2, type=float, help='threshold of stream count')
    parser.add_argument('--count-thr-type', default='Default', type=str, choices=['Increase', 'Decrease', 'Constant'])
