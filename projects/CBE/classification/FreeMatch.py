...
def main(mark, params=None):
    ...
    args.p_t = torch.ones(args.num_classes) / args.num_classes
    args.label_hist = torch.ones(args.num_classes) / args.num_classes
    args.tau_t = args.p_t.mean()
    args.lambda_sat = 1.0
    ...
    for epo in range(args.start_epoch, args.epochs):
        ...
        total_loss, labeled_loss, sat_loss, saf_loss, mask, pl_acc = train(
        ...
        fmtc = '[{}/{} | mask: {}, pl_acc: {}] total_loss: {}, labeled_loss: {}, ' \
               'sat_loss: {}, saf_loss: {}'
...
def train(labeled_loader, unlabeled_loader, model, model_ema, optimizer, scheduler, args):
    ...
    sat_loss_counter = AvgCounter()
    saf_loss_counter = AvgCounter()
    ...
    for batch_idx in range(args.eval_step):
        ...
        # region 2.4 SelfAdaptiveLoss (FreeMatch Unsupervised Loss)
        args.p_t = args.p_t.to(args.device)
        args.tau_t = args.tau_t.to(args.device)
        args.label_hist = args.label_hist.to(args.device)

        sat_loss, pseudo_label_u, mask, args.tau_t, args.p_t, args.label_hist = sat_criterion(
            logits_u_w, logits_u_s, args.tau_t, args.p_t, args.label_hist
        )
        saf_loss, hist_p_ulb_s = saf_criterion(mask, logits_u_s, args.p_t, args.label_hist)
        sat_loss_counter.update(sat_loss.item(), 1)
        saf_loss_counter.update(saf_loss.item(), 1)
        ...
        log_content += ' | loss_sat: {}'.format(format(sat_loss.item(), '.3f'))
        ...
        total_loss = labeled_loss + args.lambda_sat * sat_loss + args.lambda_saf * saf_loss
        ...
        # region 2.6 visualization
        if args.debug:
            # 2.6.1 pseudo-labels distribution visualization
    ...
    sat_loss_val = sat_loss_counter.avg
    saf_loss_val = saf_loss_counter.avg
    mask_probs_val = mask_probs.avg
    pl_acc_val = pl_acc_counter.avg
    return total_loss_val, labeled_loss_val, sat_loss_val, saf_loss_val, mask_probs_val, pl_acc_val
...
def init_args(params=None):
    ...
    # Training strategy
    parser.add_argument('--total-steps', default=1024*200, type=int, help='number of total steps to run')
...
if __name__ == '__main__':
    main('FreeMatch', {'gpu_id': 0, 'debug': True})
