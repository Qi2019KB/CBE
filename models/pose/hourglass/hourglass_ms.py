...
class StackedHourglass_MS(nn.Module):
    def __init__(self, device, ms_stream_num, is_expend, noisy_factor, k, stack_num):
        ...
        self.ms = [MultiStream(device, self.ms_stream_num, [256, 64, 64], is_expand=self.is_expend, noisy_factor=self.noisy_factor) for sIdx in range(self.stack_num)]
    ...
    def forward(self, x, student_active_params=None):
...
def build_hourglass_ms(stream_num, k, is_expend, noisy_factor, device, stack_num=3):
    return StackedHourglass_MS(device, stream_num, is_expend, noisy_factor, k, stack_num)
